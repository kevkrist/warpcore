...
class HashSet {
 ...
 public:
   ...
   */
  HOSTQUALIFIER INLINEQUALIFIER explicit HashSet(
    index_type min_capacity, key_type seed = defaults::seed<key_type>()) noexcept
    : status_(nullptr),
      keys_(nullptr),
   ...
   */
  HOSTDEVICEQUALIFIER INLINEQUALIFIER HashSet(const HashSet& o) noexcept
    : status_(o.status_),
      keys_(o.keys_),
   ...
   */
  HOSTQUALIFIER INLINEQUALIFIER HashSet(HashSet&& o) noexcept
  ...
  /*! \brief destructor
   */
  HOSTQUALIFIER INLINEQUALIFIER ~HashSet() noexcept
  {
    if (!is_copy_) {
                                          ...
                                          cudaStream_t stream = cudaStreamDefault) noexcept
  {
    ...
    if (is_initialized_) {
      kernels::memset<key_type, empty_key()>
        <<<SDIV(capacity_, WARPCORE_BLOCKSIZE), WARPCORE_BLOCKSIZE, 0, stream>>>(keys_, capacity_);
   ...
   */
  DEVICEQUALIFIER INLINEQUALIFIER Status
  insert(key_type key_in,
         ...
         index_type probing_length = defaults::probing_length()) noexcept
  {
    ...
    for (index_type i = iter.begin(key_in, seed_); i != iter.end(); i = iter.next()) {
      ...
      while (empty_mask) {
        ...
        if (group.any(success)) { return Status::none(); }
   ...
   * \param[in] filter_values pointer to values by which to filter insertion
   * \param[in] num_in number of keys to insert
   * \param[in] stream CUDA stream in which this operation is executed in
   * \param[in] probing_length maximum number of probing attempts
   * \param[out] status_out status information per key
  ...
  HOSTQUALIFIER INLINEQUALIFIER void retrieve_all(
    key_type* keys_out, index_type& num_out, cudaStream_t stream = cudaStreamDefault) const noexcept
  {
    if (!is_initialized_) return;
    ...
    for_each(
      [=, *this] DEVICEQUALIFIER(key_type key) { keys_out[helpers::atomicAggInc(tmp)] = key; },
      stream);
        ...
        index_type probing_length = defaults::probing_length()) noexcept
  {
    ...
    for (index_type i = iter.begin(key_in, seed_); i != iter.end(); i = iter.next()) {
      ...
      if (hit_mask) {
        ...
        if (group.thread_rank() == leader) { keys_[i] = tombstone_key(); }

        return Status::none();
      }

      if (group.any(is_empty_key(table_key))) { return Status::key_not_found(); }
    }
  ...
  /*! \brief applies a function on all keys inside the table
   ...
   */
  template <class Func>
  HOSTQUALIFIER INLINEQUALIFIER void for_each(Func f,
                                              cudaStream_t stream   = cudaStreamDefault,
                                              index_type smem_bytes = 0) const noexcept
  {
    if (!is_initialized_) return;

    helpers::lambda_kernel<<<SDIV(capacity(), MAXBLOCKSIZE), MAXBLOCKSIZE, smem_bytes, stream>>>(
  ...
  size(cudaStream_t stream = cudaStreamDefault) const noexcept
  {
    ...
    helpers::lambda_kernel<<<SDIV(capacity_, WARPCORE_BLOCKSIZE),
                             WARPCORE_BLOCKSIZE,
                             sizeof(index_type),
      ...
      if (block.thread_rank() == 0) { atomicAdd(tmp, smem); }
    });
   ...
   */
  HOSTDEVICEQUALIFIER INLINEQUALIFIER index_type capacity() const noexcept { return capacity_; }
   ...
   * \return the status
   */
  HOSTQUALIFIER INLINEQUALIFIER Status pop_status(cudaStream_t stream = cudaStreamDefault) noexcept
  {
    Status status = Status::not_initialized();
   ...
   * \return \c bool
   */
  HOSTDEVICEQUALIFIER INLINEQUALIFIER static constexpr bool is_empty_key(key_type key) noexcept
  {
   ...
   * \return \c bool
   */
  HOSTDEVICEQUALIFIER INLINEQUALIFIER static constexpr bool is_tombstone_key(key_type key) noexcept
  {
   ...
   * \return \c bool
   */
  HOSTDEVICEQUALIFIER INLINEQUALIFIER static constexpr bool is_valid_key(key_type key) noexcept
  {
 ...
 private:
  ...
  HOSTQUALIFIER INLINEQUALIFIER void assign_status(
    Status status, cudaStream_t stream = cudaStreamDefault) const noexcept
  {
    if (!is_initialized_) return;

    cudaMemcpyAsync(status_, &status, sizeof(Status), H2D, stream);
    ...
    Status status, cudaStream_t stream = cudaStreamDefault) const noexcept
  {
    ...
    cudaMemcpyAsync(status_, &joined, sizeof(Status), H2D, stream);
