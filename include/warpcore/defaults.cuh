 ...
 */
namespace warpcore::defaults {
template <class Key>
HOSTDEVICEQUALIFIER INLINEQUALIFIER constexpr Key empty_key() noexcept
{
  return Key{0};
}

template <class Key>
HOSTDEVICEQUALIFIER INLINEQUALIFIER constexpr Key tombstone_key() noexcept
{
  return ~Key{0};
}

template <class Key>
HOSTDEVICEQUALIFIER INLINEQUALIFIER constexpr Key seed() noexcept
{
  return Key{0x5ad0ded};
}

HOSTDEVICEQUALIFIER INLINEQUALIFIER constexpr index_t temp_memory_bytes() noexcept { return 2048; }

HOSTDEVICEQUALIFIER INLINEQUALIFIER constexpr index_t probing_length() noexcept
{
  return ~index_t(0) - 1024;
}
