...
class BloomFilter {
  ...
  static_assert(checks::is_hasher<Hasher>(), "not a valid hasher type");

  static_assert(checks::is_valid_slot_type<Slot>(), "invalid slot type");

  static_assert(checks::is_valid_cg_size(CGSize), "invalid cooperative group size");
 ...
 public:
   ...
   */
  HOSTDEVICEQUALIFIER INLINEQUALIFIER static constexpr index_type slot_bits() noexcept
  {
   ...
   * \return number of bits
   */
  HOSTDEVICEQUALIFIER INLINEQUALIFIER static constexpr index_type block_bits() noexcept
  {
   ...
   */
  HOSTQUALIFIER
  explicit BloomFilter(const index_type num_bits,
                       const index_type k,
   ...
   */
  HOSTDEVICEQUALIFIER INLINEQUALIFIER BloomFilter(const BloomFilter& o) noexcept
   ...
   */
  HOSTQUALIFIER INLINEQUALIFIER BloomFilter(BloomFilter&& o) noexcept
                                          ...
                                          const cudaStream_t stream = 0) noexcept
  {
    ...
    kernels::memset<<<SDIV(num_slots_, WARPCORE_BLOCKSIZE), WARPCORE_BLOCKSIZE, 0, stream>>>(
   ...
   */
  DEVICEQUALIFIER INLINEQUALIFIER void insert(
    const key_type key, const cg::thread_block_tile<cg_size()>& group) noexcept
  {
    const index_type slot_index =
      ((Hasher::hash(key + seed_) % num_blocks_) * cg_size() + group.thread_rank()) % num_slots_;
    ...
    if ((slot & bloom_filter_[slot_index]) != slot) { atomicOr(bloom_filter_ + slot_index, slot); }
  }
  ...
  HOSTQUALIFIER INLINEQUALIFIER void insert_if(const Key* const keys_in,
                                               Filter f,
   ...
   */
  DEVICEQUALIFIER INLINEQUALIFIER bool retrieve(
    const key_type key, const cg::thread_block_tile<cg_size()>& group) const noexcept
  {
    const index_type slot_index =
      ((Hasher::hash(key + seed_) % num_blocks_) * cg_size() + group.thread_rank()) % num_slots_;
   ...
   */
  HOSTDEVICEQUALIFIER INLINEQUALIFIER index_type num_bits() const noexcept { return num_bits_; }

  /*! \brief get number of slots
   * \return number of slots
   */
  HOSTDEVICEQUALIFIER INLINEQUALIFIER index_type num_slots() const noexcept { return num_slots_; }

  /*! \brief get number of blocks
   * \return number of blocks
   */
  HOSTDEVICEQUALIFIER INLINEQUALIFIER index_type num_blocks() const noexcept { return num_blocks_; }

  /*! \brief get number of hash functions (k)
   * \return number of hash functions (k)
   */
  HOSTDEVICEQUALIFIER INLINEQUALIFIER index_type k() const noexcept { return k_; }
   ...
   */
  HOSTQUALIFIER INLINEQUALIFIER double fpr(const index_type n) const noexcept
  {
    double res     = 0.0;
...
#pragma omp parallel for reduction(+ : res)
 ...
 private:
   ...
   */
  HOSTQUALIFIER INLINEQUALIFIER double binom(const index_type n,
                                             const index_type k,
   ...
   * \return FPR
   */
  HOSTQUALIFIER INLINEQUALIFIER double fpr_base(const index_type m,
                                                const index_type n,
