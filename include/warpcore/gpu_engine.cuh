...
namespace warpcore::kernels {
	...
	GLOBALQUALIFIER void for_each_unique_key(Func f, const Core core) {
		...
		if (gid < core.capacity()) {
			...
			if (core.is_valid_key(search_key)) {
				...
				for (index_type i = iter.begin(search_key, core.seed_); i != iter.end();
					i = iter.next()) {
	...
	GLOBALQUALIFIER void
		for_each(Func f, const typename Core::key_type* const keys_in,
			const index_t num_in, const Core core,
	...
	namespace bloom_filter {
		...
		GLOBALQUALIFIER void insert(const typename Core::key_type* const keys_in,
			const index_t num_in, Core core) {
			const index_t tid = helpers::global_thread_id();
			const index_t gid = tid / Core::cg_size();
			const auto group =
				cg::tiled_partition<Core::cg_size()>(cg::this_thread_block());
		...
		GLOBALQUALIFIER void insert_if(const typename Core::key_type* const keys_in,
			Filter f, const FilterValueType* const values_in,
			const index_t num_in, Core core) {
			const index_t tid = helpers::global_thread_id();
			const index_t gid = tid / Core::cg_size();
			const auto group =
		...
		template <class Core>
		GLOBALQUALIFIER void
			retrieve(const typename Core::key_type* const keys_in, const index_t num_in,
				typename Core::value_type* const values_out, const Core core) {
		...
		template <class Core, typename... Columns>
		GLOBALQUALIFIER void
			retrieve_write(const typename Core::key_type* const keys_in,
				const index_t num_in, typename Core::key_type* keys_out,
				int* counter, const Core core, Columns... columns) {
			const index_t tid = blockDim.x * blockIdx.x + threadIdx.x;
			const index_t gid = tid / Core::cg_size();
			const auto group =
				cg::tiled_partition<Core::cg_size()>(cg::this_thread_block());
			...
			if (gid < num_in) {
				...
				if (core.retrieve(key, group)) {
					...
					if (group.thread_rank() == 0) {
						...
						(..., (columns.out[write_index] = columns.in[gid]));
					}
				}
			}
		}

		template <class Core, typename Filter, typename FilterTypeValue, typename... Columns>
		GLOBALQUALIFIER void
			retrieve_write_if(const typename Core::key_type* const keys_in,
				Filter f, const FilterTypeValue * const filter_values,
				const index_t num_in, typename Core::key_type* keys_out,
				int* counter, const Core core, Columns... columns) {
			const index_t tid = blockDim.x * blockIdx.x + threadIdx.x;
			const index_t gid = tid / Core::cg_size();
	...
	GLOBALQUALIFIER void
		insert(const typename Core::key_type* const keys_in, const index_t num_in,
			Core core, const index_t probing_length = defaults::probing_length(),
			typename StatusHandler::base_type* const status_out = nullptr) {
		const index_t tid = helpers::global_thread_id();
		const index_t gid = tid / Core::cg_size();
		const auto group =
	...
	template <class Core, class StatusHandler = defaults::status_handler_t>
	GLOBALQUALIFIER void
		insert(const typename Core::key_type* const keys_in,
			const typename Core::value_type* const values_in, const index_t num_in,
			Core core, const index_t probing_length = defaults::probing_length(),
			typename StatusHandler::base_type* const status_out = nullptr) {
		const index_t tid = helpers::global_thread_id();
		const index_t gid = tid / Core::cg_size();
	...
	*/
	...
	GLOBALQUALIFIER void
		...
		if (gid < num_in) {
			...
			auto status =
				core.retrieve(keys_in[gid], values_out + begin_offsets_in[gid], num_out,
					group, probing_length);
	...
	template <class Core, class StatusHandler = defaults::status_handler_t>
	GLOBALQUALIFIER void
		erase(const typename Core::key_type* const keys_in, const index_t num_in,
			Core core, const index_t probing_length = defaults::probing_length(),
			typename StatusHandler::base_type* const status_out = nullptr) {
		const index_t tid = helpers::global_thread_id();
		const index_t gid = tid / Core::cg_size();
...
#pragma unroll
	...
	template <class Core, class StatusHandler = defaults::status_handler_t>
	GLOBALQUALIFIER void
		num_values(const typename Core::key_type* const keys_in, const index_t num_in,
			index_t* const num_out, index_t* const num_per_key_out,
			const Core core,
			const index_t probing_length = defaults::probing_length(),
			typename StatusHandler::base_type* const status_out = nullptr) {
		const index_t tid = helpers::global_thread_id();
